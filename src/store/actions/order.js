...
export const purchaseBurger = (obj, token, user_id) => {
  return dispatch => {
    ...
    axios
      .get("orders/" + user_id + "/orders.json?auth=" + token, obj)
      .then(response => {
...
export const fetchOrdersInit = (token, user_id) => {
  return dispatch => {
    ...
    console.log("orders/" + user_id + "/orders.json?auth=" + token);
    axios
      ...
      .then(res => {
        ...
        if (!res.data) {
          ...
          setTimeout(() => {
            dispatch(fetchOrdersFail("There are no orders in you history."));
          }, 1000);
        } else {
          const fetchOrders = [];
